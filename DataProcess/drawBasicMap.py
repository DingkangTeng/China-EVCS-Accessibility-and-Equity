...
try:
    from .setting import plotSet, INDEX, FIG_SIZE
except:
    from setting import plotSet, INDEX, FIG_SIZE
...
def stackplot(df: pd.DataFrame, path: str = "", name: str = "result") -> None:
    plotSet()
    fig = plt.figure(figsize=FIG_SIZE)
    ax = plt.subplot()
    years = list(range(2016, 2026))
    df.loc[df["name"] == "sum", years].T.plot(
        ax=ax,
        color=["teal"]
    )

    plt.ticklabel_format(style="plain", axis='y')
    plt.legend(["Number of chargers in China"])
    plt.xlabel("Year")
    plt.xticks(range(0,10), years, rotation=45) # type: ignore
    plt.ylabel("The number of chargers")
